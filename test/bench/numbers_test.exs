...
defimplEx Decimal, %Decimal{}, for: Basic, inline: [add: 2, mult: 2] do
...
defimplEx Integer, i when is_integer(i), for: Basic, inline: [add: 2, mult: 2] do
  def add(a, b) when is_integer(a), do: a + b
  def mult(a, b) when is_integer(b), do: a * b
end

defimplEx Float, f when is_float(f), for: Basic, inline: [add: 2, mult: 2] do
  def add(a, b) when is_float(a), do: a + b
  def mult(a, b) when is_float(a), do: a * b
end

defimplEx MyDecimal, {MyDecimal, s, c, e}, for: Basic, inline: [add: 2, mult: 2] do
  ...
  def add({MyDecimal, s0, c0, e0}, {MyDecimal, s1, c1, e1}) do
    {c0, c1} =
      cond do
        e0 === e1 -> {c0, c1}
        e0 > e1 -> {c0 * Basic.MyDecimal.pow10(e0 - e1), c1}
        true -> {c0, c1 * Basic.MyDecimal.pow10(e1 - e0)}
      end
    c = s0 * c0 + s1 * c1
    e = Kernel.min(e0, e1)
    s =
...
# Coerce
defimplEx IntegerFloat, {l, r} when (is_integer(l) and is_float(r)) or (is_integer(r) and is_float(l)), for: Coerce, inline: [coerce: 1] do
  def coerce({l, r}) when (is_integer(l) and is_float(r)) or (is_integer(r) and is_float(l)), do: {0.0+l, 0.0+r}
end
defimplEx Integer, {l, r} when is_integer(l) and is_integer(r), for: Coerce, inline: [coerce: 1] do
  def coerce({l, r}) when is_integer(l) and is_integer(r), do: {l, r}
end
defimplEx Float, {l, r} when is_float(l) and is_float(r), for: Coerce, inline: [coerce: 1] do
  def coerce({l, r}) when is_float(l) and is_float(r), do: {l, r}
end
defimplEx Decimal,{%Decimal{}, %Decimal{}}, for: Coerce, inline: [coerce: 1] do
  def coerce({%Decimal{}, %Decimal{}} = result), do: result
end
defimplEx MyDecimal,{{MyDecimal, _, _, _}, {MyDecimal, _, _, _}}, for: Coerce, inline: [coerce: 1] do
  def coerce({{MyDecimal, _, _, _}, {MyDecimal, _, _, _}} = result), do: result
end


defmodule NumbersResolved do
  resolveProtocolEx(Basic, [
    Integer,
    Float,
    Decimal,
    MyDecimal,
...
defmodule ProtocolEx.Bench.NumbersTest do
  ...
  test "Numbers - Bench" do
    ...
    bench = %{
      "MyNumbers"   => fn {l, r} -> {l, r} = Coerce.coerce(l, r); Basic.add(l, r) end,
      "MyNumbers - sans coerce"   => fn {l, r} ->Basic.add(l, r) end,
      "Numbers"     => fn {l, r} -> Numbers.add(l, r) end,
      "Numbers - sans coerce"     => fn {l, r} -> Numbers.Protocols.Addition.add(l, r) end,
      "Numbers - my coerce"     => fn {l, r} -> {l, r} = Coerce.coerce(l, r); Numbers.Protocols.Addition.add(l, r) end,
    }
